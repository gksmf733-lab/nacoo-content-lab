...
// 일반적인 파트너센터 공지 페이지 후보 — 실제 경로 확정 후 수정
const CANDIDATE_URLS = [
  "https://partner.booking.naver.com/notice/list",
  "https://partner.booking.naver.com/#/notice",
...
export async function scrape({ recentDays = 30 } = {}) {
  ...
  try {
    ...
    console.error(`  ✗ 예약 파트너 로그인 실패: ${err.message}`);
  ...
  try {
    ...
    if (!loaded) {
      console.error("  ⚠ 예약 파트너 공지 페이지를 찾지 못함 — URL 확정 필요");
      return [];
    }
    ...
    if (list.length === 0) {
      console.error("  ⚠ 예약 파트너 공지 0건 — DOM 셀렉터 조정 필요");
      return [];
    }
